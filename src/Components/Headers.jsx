...
const Header = () => {
  ...
  return (
    <header className="bg-white shadow-xl border-b border-gray-200 sticky top-0 z-50">
      <div className="w-10/12 mx-auto px-4 sm:px-6 my-4">
        <div className="flex items-center justify-between h-16 w-full">      
          ...
          <div className="flex items-center space-x-2 lg:space-x-4">
            <Link to="/location">
              <button type="button" className="hidden lg:flex items-center space-x-2 text-black hover:text-green-600 font-semibold text-sm transition-colors duration-200 px-2 lg:px-3 py-2 rounded-full hover:bg-gray-50">
      ...
      {isMenuOpen && <div className="lg:hidden fixed left-0 inset-0 top-24 opacity-50 bg-black" onClick={toggleMenu}></div>}
      <div className={`lg:hidden fixed top-24 right-0 h-screen w-80 bg-white shadow-2xl transition-transform duration-300 z-50 ${isMenuOpen ? 'translate-x-0' : 'translate-x-full'}`}>
        <div className="px-6 py-4">
          ...
          <Link to="/location">
            <button type="button"className="flex items-center space-x-3 text-gray-800 hover:text-green-600 font-medium py-2 w-full text-left"
